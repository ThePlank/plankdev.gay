...
-------- currently using xkcd colour naming conventions ---------

io.write('input the fucking hex colour: ')
local hex = io.read(7)
local palette = io.open('rgbxkcd.txt', 'r')
...
for colour in palette:lines('l') do
	...
	for destName, destHex in colour:gmatch('([%a%s]+)%s*#([0-9a-fA-F]+)') do
		local destR, destG, destB = tonumber("0x"..destHex:sub(1,2)), tonumber("0x"..destHex:sub(3,4)), tonumber("0x"..destHex:sub(5,6))
